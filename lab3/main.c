...
int simulation_length = DEFAULT_SIM_LENGTH;				    // default to 30 seconds
...
client(void *arg) {
    ...
    // Initialize the prng [pseudo random number generator].  
    // For testing, it may be helpful to reduce noise by 
...
int main(int argc, char ** argv) {
    ...
    init(numthreads);
    srandom(time(0)); //randomizes based on the current time
