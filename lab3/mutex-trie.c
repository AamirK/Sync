...
struct trie_node {
    ...
    char key[MAX_KEY_SIZE];							    /* Up to MAX_KEY_SIZE chars */
} ;
...
static struct trie_node * new_leaf(const char *string, size_t strlen, int32_t ip4_address) {
...
static int compare_keys(const char *string1, int len1, const char *string2, int len2, int *pKeylen) {
...
int delete(const char *string, size_t strlen) {
    ...
    rv = (_delete(root, string, strlen)) == NULL ? 0 : 1;
    pthread_mutex_unlock(&trie_mutex);
    print();
    return rv;
}
